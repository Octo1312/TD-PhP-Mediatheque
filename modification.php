...
<head>
    ...
    <title>Modification fichefilm</title>
</head>
...
<body>
    ...
    <main>
        <h2>Modification de fiche film</h2>
        <?php 
        $bdd = new PDO('mysql:host=localhost;dbname=mediatheque;charset=utf8', 'root');
        $requestinfo = $bdd->prepare('SELECT * FROM fiche_film WHERE id = :id');
        $requestinfo->execute(['id' => $_GET['id']]);
        $fiche_film = $requestinfo->fetch();
        ?>
        <form action="filmmaker.php" method="POST" enctype="multipart/form-data">
            <label for="title">Titre du film :</label>
            <input type="text" name="titre" value="<?=$fiche_film['titre']?>">
            <label for="realisator">Réalisateur :</label>
            <input type="text" name="realisateur" value="<?=$fiche_film['realisateur']?>">
            <label for="type">Genre du film :</label>
            <input type="text" name="genre" value="<?=$fiche_film['genre']?>">
            <label for="duree">Temps du film :</label>
            <input type="text" name="duree" value="<?=$fiche_film['duree']?>">
            <label for="syno">Synopsis :</label>
            <input type="text" name="synopsis" value="<?=$fiche_film['synopsis']?>">
